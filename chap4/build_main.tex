...
% Pado: Since the data collection was technically done outside of the thesis period, 4.2.1 (Multiple cycles) can be omitted. 4.2.2 is needed in order to understand the data set.
...
% Pado: This is a little confusing - does the Y axis show the force? Why is the blue line the shape it is? Is it maybe two separate lines, the movement of the punch and the reaction of the metal? 
...
\paragraph*{$n\_estimators$} Sets the number of decision trees to construct.
Was set to X determined by the grid search.

\paragraph*{$max\_depth$}
...
\paragraph*{$min\_samples\_split$}

\paragraph*{$min\_samples\_leaf$}

\paragraph*{$bootstrap$}

\paragraph*{$criterion$} Is set to "mse" which stands for mean squared error. 
The dataset only contains continuous numerical values and therefore the mean squared error is the best choice for the criterion. 
% TODO: Elaborate on this

\paragraph*{$max\_features$} Is set to "auto" which means that the algorithm uses all features for training the model. As shown in Figure~\ref{fig:rf_feature_importance} all features are important for the prediction of the spring back. Therefore, all features are used for training the model.
