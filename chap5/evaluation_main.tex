...
The goal of this section is to provide a recommendation for the use in practise and is the target groups are
practitioners.
To apply this study closely to practise for each \ac{DP}(s)  a concrete example use case is provided
followed by a recommendation for the use of the models based on the results of the study.

\textbf{Correctness: High precision metal components}:

Precision is very important in certain industries, such as aerospace, where even minor inaccuracies in metal
production can result in critical failures.
Therefore, the development of accurate \ac{ML} models is essential for predicting spring back in these applications.
The recommended \ac{ML} models for this use case are the \ac{SVM} and \ac{MLP}based on their proven effectiveness in
predicting the desired outcomes.


\textbf{Relevance \& Stability: Diverse metal components}:

When working with a wide variety of metal components that differ in thicknesses, bending angles, or types of metal,
\ac{ML} models need to be adaptable to these different conditions.
...
Among the best models in terms of relevance were \ac{SVM}, \ac{ET}, and \ac{MLP}.
However, the SVM and MLP models did not prove to be stable and were among the low-performing
models in that regard.
Therefore, the recommended model for this use case is the ET model.

\textbf{Robustness: Manufacturing with varying data quality}:
In some industries, such as the automotive industry, data quality may vary due to factors such as wear and tear on
machinery or differences in material batches.
Therefore, an \ac{ML} model that is robust would be essential in these scenarios as it could handle variations in the
...
In the missing values test, the \ac{MLP}, followed by the \ac{SVM}, seemed to be the best performing models.
However, when noise was added to the training data, a different picture emerged, with both models performing the
worst compared to the other tree-based models.

As the variance in the missing values test was not very high, as seen in \cref{fig:variance-missing-values}, it is
advisable to prioritize the noise test.
Therefore, we recommend selecting the \ac{ET} and \ac{GBT} models for this case.


\textbf{Resource utilization: Small-scale manufacturing}
...
\textbf{Interpretability: Collaborative projects with non-experts}:
In many projects some stakeholders or users doe not have deep understand of \ac{ML}, therefore it is important that
a \ac{ML} can explain it's predictions.
An interpretable model can help to build trust in the model and the predictions and can help to identify errors in the
data or the model.

Since only model-agnostic methods where used in this study the results are not very conclusive.
All models can be recommended for this use cas.
